...
export default function Calendar() {
  ...
  const fetchEvents = () => {
    ...
    eventsAPI.list(start, end).then((r) => {
      setEvents(r.data);
      setLoading(false);
    });
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="btn-primary flex items-center gap-2" onClick={() => { setForm({ title: '', description: '', start_time: '', end_time: '', location: '' }); setShowModal(true); }}>
          ...
          <>
            ...
            <div className="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
              {Array.from({ length: firstDay }).map((_, i) => (
                <div key={`empty-${i}`} className="bg-gray-50 dark:bg-gray-800 h-24" />
              ))}
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white dark:bg-gray-800 h-24 p-1 cursor-pointer hover:bg-jarvis-50 dark:hover:bg-gray-750 transition-colors ${
                      isToday ? 'ring-2 ring-inset ring-jarvis-500' : ''
                    }`}
                  >
                    <span className={`text-sm font-medium ${isToday ? 'text-jarvis-600' : ''}`}>{day}</span>
                    <div className="mt-1 space-y-0.5">
                      {dayEvents.slice(0, 3).map((ev) => (
                        <div key={ev.id} className="text-xs bg-jarvis-100 dark:bg-jarvis-900/40 text-jarvis-700 dark:text-jarvis-300 px-1 rounded truncate">
                          {ev.title}
                        </div>
                      ))}
        ...
        <form onSubmit={handleCreate} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className="input mt-1" type="datetime-local" value={form.start_time} onChange={(e) => setForm({ ...form, start_time: e.target.value })} required />
            </div>
            <div>
              <label className="text-sm font-medium">End</label>
              <input className="input mt-1" type="datetime-local" value={form.end_time} onChange={(e) => setForm({ ...form, end_time: e.target.value })} required />
            </div>
          </div>
